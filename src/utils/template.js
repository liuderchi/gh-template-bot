...
const getCommand /*: string => Command */ = issueBody => {
  ...
  const getActionInDialog /*: string => string */ = R.pipe(
    R.match(cmdRegexForDialog),
    R.path(['2'])
  );
  ...
  return {
    action: R.ifElse(
      getActionInDialog,
      R.pipe(getActionInDialog, action => action.toUpperCase()),
      getAction
    )(issueBody),
...
const matchAction /*: string => Template => boolean */ = action => ({ name }) =>
  name.toLowerCase() === `${action.toLowerCase()}.md`;

const validateAction /*: (Command, Array<Template>) => void */ = (
  command,
  customTemplates
) => {
...
const matchIssueTemplate /*: Template => boolean */ = ({ name }) =>
  R.test(/ISSUE_TEMPLATE/i, name);

const matchPRTemplate /*: Template => boolean */ = ({ name }) =>
  R.test(/PULL_REQUEST_TEMPLATE/i, name);

const insertTemplate /*: (Object, string, string) => string */ = (
  options,
  template,
  issueBody
) => {
...
const readTemplate /*: string => string */ = fileName =>
  fs.readFileSync(path.join(__dirname, '../templates', fileName), 'utf8');

const getMDContent /*: (Command, Array<Template>, string) => Promise<string> */ = async (
  command,
  customTemplates = [],
  issueBody
) => {
  ...
  switch (action.toUpperCase()) {
    ...
    case 'ISSUE':
      if (customTemplates.some(matchIssueTemplate)) {
        const { download_url } = customTemplates
          .filter(matchIssueTemplate)
          .shift();
